...
data RedoNode = RedoNode {
  inFiles :: [RedoTarget],
  outFiles :: Int
  } deriving Show

buildGraph :: Dependency -> RedoTarget -> StateT (M.Map RedoTarget RedoNode) IO ()
buildGraph dep@(ExistingDependency target oldSig) from = do
  unchanged <- liftIO $ upToDate dep
  unless unchanged $ do
    maybeDeps <- liftIO $ getDependencies target
    case maybeDeps of
      Nothing -> do
        return ()
      Just [] -> liftIO . throwIO $ InvalidDependency target
      (Just deps) -> do
        p <- liftIO $ upToDate (head deps)
        if p
          then do
            dd <- liftIO $ filterM upToDate (tail deps)
            return ()
          else
            return ()

data RedoTargetStatus =
  TargetUpToDate |
  TargetScriptOutdated |
  TargetSourceOutdated
  deriving Show

isTargetUpToDate TargetUpToDate = True
isTargetUpToDate _ = False

isTargetSourceOutdated TargetSourceOutdated = True
isTargetSourceOutdated _ = False

isTargetScriptOutdated TargetScriptOutdated = True
isTargetScriptOutdated _ = False

redoRequired :: RedoTarget -> IO Bool
redoRequired target = do
  ...
  case maybeDeps of
    ...
    (Just deps@(ExistingDependency aDo _:_)) -> do
      ...
      let deps' = map NonExistingDependency exDos ++ deps
      changed <- and <$> mapM dependencyChanged deps'
      if changed
        then return True
        else and <$> mapM redoRequired (redoTargetFrom deps)
    _ -> throwIO $ InvalidDependency target

redoTargetFrom :: [Dependency] -> [RedoTarget]
redoTargetFrom (ExistingDependency target _:rest) = target : redoTargetFrom rest
redoTargetFrom (_:rest) = redoTargetFrom rest
redoTargetFrom [] = []

dependencyChanged :: Dependency -> IO Bool
dependencyChanged (ExistingDependency target oldSig) = do
  newSig <- fileSignature target
  case newSig of
    -- target does not exist.
    NoSignature -> return True
    _ -> return $ oldSig /= newSig
dependencyChanged (NonExistingDependency target) = doesFileExist target
